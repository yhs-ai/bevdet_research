...
@PIPELINES.register_module()
class LoadMultiViewImageFromFiles_BEVDetTemporal(object):
    """Load multi channel images from a list of separate channel files.
    ...
    def __init__(self, data_config, is_train=False,
        ...
        #self.nusc = NuScenes(version='v1.0-trainval', dataroot='/media/yhson/2c4a7dd4-f03e-462b-a004-60607e96489a/bdkim/data/nuscenes/', verbose=True)
    ...
    def get_inputs(self,results, flip=None, scale=None):
        ...
        for cam in cams:
            ...
            ############## chgd
            token_list = []
            sample_data_token = cam_data['sample_data_token']
            cali_token = self.nusc.get('sample_data',sample_data_token)['calibrated_sensor_token']
            sens = self.nusc.get('calibrated_sensor', cali_token)

            token_list.append(sens)
            ##############
            '''
        ...
        # chgd for temporal (handle t-1 and t-2)
        prev_imgs = []
        prev_post_rots = []
        prev_post_trans = []
        prev_img_idx = results['prev_img_idx']

        for prv_img in prev_img_idx:
            img = Image.open(prv_img)
            post_rot = torch.eye(2)
            post_tran = torch.zeros(2)
            ...
            # for convenience, make augmentation matrices 3x3
            prev_post_tran = torch.zeros(3)
            prev_post_rot = torch.eye(3)
            prev_post_tran[:2] = post_tran2
            prev_post_rot[:2, :2] = post_rot2


            prev_imgs.append(self.normalize_img(img))
            prev_post_rots.append(prev_post_rot)
            prev_post_trans.append(prev_post_tran)


        #pdb.set_trace()
        '''
        prev_intrins = torch.Tensor(results['prev_intrins'])
        prev_rots = torch.Tensor(results['prev_rots'])
        prev_trans = torch.Tensor(results['prev_trans'])
        '''
        prev_intrins = results['prev_intrins']
        prev_rots = results['prev_rots']
        prev_trans = results['prev_trans']
        ...
        imgs, rots, trans, intrins, post_rots, post_trans = (torch.stack(imgs), torch.stack(rots), torch.stack(trans),
                                                             ...
                                                             torch.stack(post_trans))
        # return prev infos
        prev_imgs, prev_rots, prev_trans, prev_intrins, prev_post_rots, prev_post_trans = \
                                    (torch.stack(prev_imgs), torch.stack(prev_rots), torch.stack(prev_trans), 
                                     torch.stack(prev_intrins), torch.stack(prev_post_rots), torch.stack(prev_post_trans))

        #pdb.set_trace()
        return imgs, rots, trans, intrins, post_rots, post_trans, prev_imgs, prev_rots, prev_trans, prev_intrins, prev_post_rots, prev_post_trans
...
class LoadMultiViewImageFromFiles_BEVDetTemporal_V2(object):
    ...
    def get_inputs(self,results, flip=None, scale=None):
        ...
        # chgd
        prev_intrins = results['prev_intrins']
        prev_rots = results['prev_rots'] # list of len 12 (3x3)
        prev_trans = results['prev_trans']
        new_prev_rots = []
        new_prev_trans = []
        ...
        for pre_tran, pre_rot in zip(prev_trans, prev_rots):
            ...
            egocurr2global = np.eye(4, dtype=np.float32)
            egocurr2global[:3,:3] = Quaternion(results['curr_e2g_rots']).rotation_matrix
            egocurr2global[:3,3] = results['curr_e2g_trans']

            lidar2ego = np.eye(4, dtype=np.float32)
            lidar2ego[:3, :3] = Quaternion(results['curr_l2e_rots']).rotation_matrix
            lidar2ego[:3, 3] = results['curr_l2e_trans']
            ...
            mat[:3,3] = pre_tran
            mat = lidaradj2lidarcurr @ mat
            new_prev_rots.append(torch.from_numpy(mat[:3,:3]))
            new_prev_trans.append(torch.from_numpy(mat[:3,3]))
            idx += 1
        ...
        # return prev infos
        prev_imgs, prev_rots, prev_trans, prev_intrins, prev_post_rots, prev_post_trans = \
                                    (torch.stack(prev_imgs), torch.stack(new_prev_rots), torch.stack(new_prev_trans), 
                                     torch.stack(prev_intrins), torch.stack(prev_post_rots), torch.stack(prev_post_trans))
